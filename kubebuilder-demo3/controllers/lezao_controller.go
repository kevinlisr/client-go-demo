...
import (
	"context"
	"github.com/kubebuilder-demo3/controllers/utils"
	...
	lecangv1beta1 "github.com/kubebuilder-demo3/api/v1beta1"
)

// LezaoReconciler reconciles a Lezao object
type LezaoReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

//+kubebuilder:rbac:groups=lecang,resources=deployments,verbs=get;list;watch;create;update;patch;delete
...
// the Lezao object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.11.2/pkg/reconcile
func (r *LezaoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	logger := log.FromContext(ctx)
	app := &lecangv1beta1.Lezao{}
	//从缓存中获取app
	if err := r.Get(ctx, req.NamespacedName, app); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}
	//根据app的配置进行处理
	//1. Deployment的处理,jiang app yu deployment zuo guan lian guan xi.
...
func (r *LezaoReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&lecangv1beta1.Lezao{}).
		Owns(&v1.Deployment{}).
		Owns(&netv1.Ingress{}).
		Owns(&corev1.Service{}).
		Complete(r)
